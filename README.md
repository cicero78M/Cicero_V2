...
2. **Copy `.env.example` to `.env`** and update the values:
   ...
   `GATEWAY_WA_CLIENT_ID` dinormalisasi ke huruf kecil (trim + lowercase). Pastikan casing konsisten dengan folder `session-<clientId>` yang sudah ada agar tidak membuat session baru; jika sebelumnya memakai huruf besar, ubah env dan/atau rename folder session supaya cocok.
