...
- `src/cron/cronDirRequestCustomSequence.js` registers its own dirRequest job key with schedules at **15:00**, **18:00**, and **20:30** (Asia/Jakarta). Each run chains the social-media fetch (posts/likes/comments), then sends Ditbinmas menus 6 (IG likes), 9 (TikTok comments), 34 (weekly/monthly recap variants), and 35 (Excel recap variants) to Ditbinmas super admins, plus menu 30 (kasatker recap) to Ditbinmas operators using the appropriate daily/weekly/monthly context. Admin WhatsApp numbers receive a short status summary after the sequence finishes. A dedicated Ditbinmas recap job also runs at **20:30** with the same Ditbinmas-only menu set and context rules.
