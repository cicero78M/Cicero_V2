...
- A parallel BIDHUMAS-only dispatch now runs at **20:30** using the same menu 6 & 9 builder, scheduled separately so it does not block the Ditbinmas recap that fires at the same minute. The combined custom sequence also runs at **20:30**, aligning with the recap while keeping BIDHUMAS and Ditbinmas flows decoupled.
