...
export async function refreshAggregator(req, res) {
  try {
    ...
    const limit = Number.isNaN(limitRequest) ? 10 : limitRequest;
    const periode = (req.body?.periode || req.query.periode || "harian").toLowerCase();
    const clientId = req.body?.client_id || req.query.client_id || null;

    const refreshed = await refreshAggregatorData({
      clientId,
      periode,
      limit,
    ...
    sendConsoleDebug({
      tag: "AGG",
      msg: `refreshAggregator success count=${refreshed.length}`,
    });

    sendSuccess(res, { message: "Aggregator refreshed", results: refreshed });
  } catch (err) {
    sendConsoleDebug({ tag: "AGG", msg: `Error refreshAggregator: ${err.message}` });
    const statusCode = /client not found|kriteria/i.test(err.message) ? 400 : 500;
