...
cron.schedule('06 6-22 * * *', async () => {
  ...
  try {
    ...
    debugMsg += `Tanggal: ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}\n`;
    ...
    for (const admin of getAdminWAIds()) {
      try { await waClient.sendMessage(admin, debugMsg); } catch (e) {}
    }
  ...
  try {
    ...
    for (const client of clients) {
      try {
        ...
        // Kirim ke console dan semua admin
        console.log(debugMsg);
        for (const admin of getAdminWAIds()) {
          try {
            await waClient.sendMessage(admin, debugMsg);
          } catch (waErr) {
            console.error(`[DEBUG WA ERROR][${client.client_id}] ke ${admin}:`, waErr.message);
          }
        }
      } catch (e) {
