...
waClient.on("message", async (msg) => {
  ...
  const waId = pengirim.startsWith("62") ? pengirim : "62" + pengirim.replace(/^0/, "");
  const res = await pool.query(q, [waId]);
  if (!res.rows || !res.rows[0]) {
    await waClient.sendMessage(chatId, "Menu ini hanya dapat diakses oleh operator client.");
    return;
  }
  session.menu = "oprrequest";
  session.step = "main";
  await oprRequestHandlers.main(session, chatId, text, waClient, pool, userService);
  return;
}
...
// ===== Handler utama operator menu =====
const oprRequestHandlers = {
  main: async (session, chatId, text, waClient, pool, userService) => {
    // Menu utama operator
    let msg =
