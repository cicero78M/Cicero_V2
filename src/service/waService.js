...
import { fetchAndStoreTiktokContent, fetchAllTikTokCommentsToday } from "./tiktokFetchService.js";
...
if (text.toLowerCase().startsWith("fetchtiktok#")) {
  ...
  await waClient.sendMessage(chatId, `⏳ Memulai fetch TikTok & komentar untuk *${client_id}* ...`);
  ...
  try {
    ...
    if (!posts || posts.length === 0) {
      await waClient.sendMessage(chatId, `Tidak ada post TikTok hari ini untuk client *${client_id}*.`);
      return;
    }

    // 2. Untuk setiap video, fetch & simpan semua komentar (paginasi semua halaman)
    let totalCommented = 0, totalVideos = 0;
    let rekap = [];
    for (const post of posts) {
      // fetchAllTikTokCommentsToday otomatis handle paginasi untuk 1 video_id
      const komentarData = await fetchAllTikTokCommentsToday(client_id, post.video_id);
    ...
    rekap.forEach((item, i) => {
      ...
      msg += `   Like: ${item.digg_count || 0} | Komentar: ${item.comment_count}\n`;
      msg += `   Link: ${item.url}\n\n`;
    });

    await waClient.sendMessage(chatId, msg.trim());
  } catch (err) {
    await waClient.sendMessage(chatId, `❌ Gagal fetch TikTok: ${err.message}`);
  }
  return;
}
