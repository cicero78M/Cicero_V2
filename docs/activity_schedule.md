...
| `cronDirRequestCustomSequence.js` (custom menus) | `0 15 * * *<br>0 18 * * *` | Chain sosmed fetches, run Ditsamapta menu 6/9 (plus optional extras) to its group, super admins, and operators, deliver Ditbinmas menu 21 to the Ditbinmas group, then send BIDHUMAS menus 6 and 9 to the BIDHUMAS group and its super admins. |
...
| `cronDirRequestCustomSequence.js` (BIDHUMAS 20:30) | `30 20 * * *` | Deliver BIDHUMAS menus 6 and 9 at the same time as the Ditbinmas recap without blocking it, targeting the BIDHUMAS group and super admins. |
