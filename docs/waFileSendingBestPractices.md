...
4. **Normalisasi & Hidratasi Chat ID Sebelum Kirim Pesan**  
   ...
   `safeSendMessage` juga mendukung **fallback client** (order: WA-GATEWAY → WA → WA-USER) bila `waClient` menyediakan `sendFallbackChain`, sehingga pengiriman otomatis mencoba gateway dahulu lalu turun ke WA utama dan WA user jika gagal.
